...
def merge_images(images_paths,results_folder,output):
    if(len(images_paths) == 0): 
        ...
        debug_center.show_in_console(None,"Error","".join(["ERROR: ", "Too few plots (2 required): ", str(len(images_paths))]))
        return -1
    elif(len(images_paths) == 3):
        debug_center.show_in_console(None,"Warning","".join(["WARNING: ", "Too many plots given (2 expected): ", str(len(images_paths))]))
        ...
        debug_center.show_in_console(None,"Warning","".join(["WARNING: ", "Too many plots given (4 expected): ", str(len(images_paths))]))
        ...
        debug_center.show_in_console(None,"Warning","".join(["WARNING: ", "Too many plots given (6 expected): ", str(len(images_paths))]))
    ...
    try:
        ...
        debug_center.show_in_console(None,"Error","ERROR: Could not run PIL to merge plots: {0} {1}".format(sys.exc_info()[0],e))
        return -1
...
def run_script(args):
    ...
    if(len(args) % 2 == 0 and len(args) < 4):
        ...
        if terminal_type != "svg": 
            result_merged_segmentation = merge_images([p for p in images_paths if p.endswith(SEGPLOT_SUFFIX)],results_folder,os.path.join(results_folder,"Plot_segmentation.png"))
            run_show_results("Segmentation plots merged", "Segmentation plots NOT merged",result_merged_segmentation)
            
            result_merged_tracking = merge_images([p for p in images_paths if p.endswith(TRACKPLOT_SUFFIX)],results_folder,os.path.join(results_folder,"Plot_tracking.png")) 
            run_show_results("Tracking plots merged", "Tracking plots NOT merged",result_merged_tracking)
        else:
            debug_center.show_in_console(None,"Info","Skipping merging plots as it is unavailable for '" + terminal_type + "' plotting terminal.")
        ...
        plot_success = create_additional_plots("F value for segmentation",segmentation_data,3,os.path.join(results_folder,"Plot_segmentation_F_summary.png"))
        run_show_results("Segmentation F value comparison plot was created...", "Segmentation F value comparison plot was NOT created...",plot_success)
        
        plot_success = create_additional_plots("Precision of segmentation",segmentation_data,1, os.path.join(results_folder,"Plot_segmentation_precision_summary.png"))
        run_show_results("Segmentation precision comparison plot was created...", "Segmentation precision comparison plot was NOT created...",plot_success)
        
        plot_success = create_additional_plots("Recall value for segmentation",segmentation_data,2,os.path.join(results_folder,"Plot_segmentation_recall_summary.png"))
        run_show_results("Segmentation recall value comparison plot was created...", "Segmentation recall value comparison plot was NOT created...",plot_success)
        
        # F value for all algorithms for tracking
        # Precision
        # Recall
        plot_success = create_additional_plots("F value for tracking",tracking_data,2,os.path.join(results_folder,"Plot_tracking_F_summary.png"))
        run_show_results("Tracking F value comparison plot was created", "Tracking F value comparison plot was NOT created",plot_success)
        
        plot_success = create_additional_plots("Precision of tracking",tracking_data,0, os.path.join(results_folder,"Plot_tracking_precision_summary.png"))
        run_show_results("Tracking precision comparison plot was created...", "Tracking precision comparison plot was NOT created...",plot_success)
        
        plot_success = create_additional_plots("Recall value for tracking",tracking_data,1,os.path.join(results_folder,"Plot_tracking_recall_summary.png"))
        run_show_results("Tracking recall value comparison plot was created...", "Tracking recall value comparison plot was NOT created...",plot_success)
