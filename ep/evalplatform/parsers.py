...
class DefaultPlatformParser(CSVCellParser):
    ...
    map_colour_name_segtrack = {"frame_nr": 0, "cell_nr": 1, "cell_colour": 2, "position_x": 3, "position_y": 4,
                                "unique_id": 5}
    ...
    def configure(self, headers):
        ...
        if any([h not in self.possible_headers for h in header_list]) or any(
                [h not in header_list for h in self.required_headers_list]):
