...
def find_all_created_files_paths(folder):
    files = [f for f in os.listdir(folder) if
             os.path.isfile(os.path.join(folder, f)) and ((TMP_SUFFIX in f) or any([s in f for s in ALL_SUFFIXES]))]
    return [os.path.join(folder, f) for f in files]


def find_all_created_directories(folder):
    directories = [f for f in os.listdir(folder) if
                   os.path.isdir(os.path.join(folder, f)) and any([s in f for s in ALL_DIRECTORIES])]
...
if __name__ == '__main__':
    ...
    if delete_tmp:
        ...
        tmp_paths_to_remove = [determine_output_filepath(file, OUTPUT_FOLDER) for file in gt_files + algo_files if
                               any([suf for suf in MERGED_SUFFIXES if file.endswith(suf)])]
