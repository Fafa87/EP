...
def find_eval(folder):
    ...
    if len(summary) == 0:
        ...
        debug_center.show_in_console(None, "Error",
                                     "ERROR: Only one summary file expected in {}, found: {}".format(folder,
                                                                                                     len(summary)))
        return ''
...
if (len(sys.argv) < 3):
    ...
    path_file = [[algorithm, find_eval(os.path.join(results_folder, algorithm))] for algorithm in algorithm_results if
                 find_eval(os.path.join(results_folder, algorithm)) != '']
